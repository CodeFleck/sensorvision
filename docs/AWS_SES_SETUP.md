# AWS SES Setup Guide for SensorVision
...
aws ses verify-email-identity \
    --email-address noreply@sensorvision.io \
    --region us-west-2
```

**Then**:
1. Check the email inbox for `noreply@sensorvision.io`
...
2. Add the TXT record to your domain's DNS:
   ```
   Name: _amazonses.sensorvision.io
   Type: TXT
...
3. Fill out the form:
   ...
   - **Use Case Description**:
     ```
     SensorVision is an IoT monitoring platform that sends transactional emails:
...
3. Enter IAM User Name: `sensorvision-ses-smtp-user`
4. Click **"Create"**
...
Name: _dmarc
Type: TXT
Value: v=DMARC1; p=none; rua=mailto:postmaster@sensorvision.io
```
...
aws ses send-email \
    ...
    --message "Subject={Data='Test Email',Charset='UTF-8'},Body={Text={Data='This is a test email from SensorVision.',Charset='UTF-8'}}" \
    --region us-west-2
```
...
public ResponseEntity<String> testEmail() {
    ...
    message.setSubject("SensorVision Test Email");
    message.setText("This is a test email from SensorVision using AWS SES.");
...
aws sns create-topic --name sensorvision-ses-bounces --region us-west-2

# Configure SES to send bounce notifications
aws ses set-identity-notification-topic \
    --identity noreply@35.88.65.186.nip.io \
    --notification-type Bounce \
    --sns-topic arn:aws:sns:us-west-2:ACCOUNT_ID:sensorvision-ses-bounces \
    --region us-west-2
```
...
**For SensorVision**: With ~1,000-5,000 emails/month, cost is **$0** (free tier).
...
2. **Add custom domain** - Replace nip.io with sensorvision.io
