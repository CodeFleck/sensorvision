...
from sensorvision import (
    SensorVisionClient,
    AsyncSensorVisionClient,
    RateLimitError,
    ServerError,
)
from sensorvision.models import TelemetryData
from sensorvision.utils import validate_device_id, validate_telemetry_data


@pytest.fixture
def sync_client():
    client = SensorVisionClient(
        api_url="http://api.sensorvision.local",
        api_key="test-token",
        retry_attempts=3,
        retry_delay=0,
        timeout=1,
...
def test_send_data_success(sync_client):
    responses.add(
        responses.POST,
        "http://api.sensorvision.local/api/v1/ingest/device-123",
        json={
            "success": True,
